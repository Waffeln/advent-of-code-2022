...
readFile('./day04b/src/assets/input.txt', 'utf-8', (err: any, data: string) => {
  if (err !== null) {
    console.error(err)
  }
  ...
  cleaningSectionArray.forEach((el: string): void => {
    ...
    if ((Number(sections[0].split('-')[0]) <= Number(sections[1].split('-')[0]) &&
      Number(sections[0].split('-')[1]) >= Number(sections[1].split('-')[0])) ||
      (Number(sections[1].split('-')[0]) <= Number(sections[0].split('-')[0]) &&
      Number(sections[1].split('-')[1]) >= Number(sections[0].split('-')[0]))) completeOverlapCount++
  })
